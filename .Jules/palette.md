...
## 2026-08-17 - Material Design 3 Dialog Polish: Icon Slots and Typography\n\n**Learning:** When styling Flutter AlertDialog widgets for Material Design 3, avoid manually nesting icons and text inside a Row for the `title` property. Instead, fully utilize the native `icon` property on `AlertDialog`, which correctly manages alignment and spacing. Additionally, styling titles with `textAlign: TextAlign.center` and the `theme.textTheme.headlineSmall` scale combined with a font weight of `w800` drastically improves the structural flow and readability of modal dialogs across varying content densities.\n\n**Action:** Refactored multiple AI and onboarding instructional dialogs (`AIMarkdownDialog`, `AICliDialog`, `AIUpdateSummaryDialog`, `ApiKeyInstructionsDialog`) to utilize the native `AlertDialog.icon` parameter and standardized MD3 headline typography.
