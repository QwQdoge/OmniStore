...
## 2026-08-07 - Codebase MD3 Cleanup & Bug Fixes

**Learning:** When resolving Git merge conflicts or making mass codebase styling updates (e.g. implementing MD3 standard dropdown containers in `add_source_dialog.dart` and `welcome_ai_page.dart`), you must ensure that duplicate variable declarations are not inadvertently introduced in the surrounding codebase sections (such as in `category_page.dart`, `discovery_content.dart`, or `home_page.dart`). Using `flutter analyze` immediately after resolving conflicts ensures no syntactical or duplicate-declaration errors are merged. Also, when establishing standardization rules (like clipping clipboard SnackBars to 2 seconds instead of the global 4-second default), explicitly pass the shorter duration to ensure correct transient feedback pacing.

**Action:** Standardized MD3 configurations for legacy inline `DropdownButton`s inside onboarding wizards, resolved stray Git merge markers in `add_source_dialog.dart`, corrected explicit duration overrides on clipboard SnackBars, and ensured the project compiles cleanly using `dart format` and `flutter analyze`.
