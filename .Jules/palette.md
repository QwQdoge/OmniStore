...
## 2026-08-08 - MD3 Dialog Secondary Actions Standardization

**Learning:** While `TextButton` is the framework default for secondary dialog actions, strict adherence to Material Design 3 guidelines for desktop density often benefits from slightly elevated visual prominence for cancellation actions, especially when paired with prominent primary `FilledButton`s. Standardizing secondary dialog actions to `FilledButton.tonal` across the application (e.g., in `AddSourceDialog`, `ImportPackagesDialog`, and various `action_dialogs.dart`) unifies the interaction language and provides clearer, more accessible touch targets without the visual noise of outlined borders.

**Action:** When creating or refining custom MD3 dialogs, consistently utilize `FilledButton` for primary confirmation actions and `FilledButton.tonal` for secondary or cancellation actions to maintain cohesive app-wide visual weight.
