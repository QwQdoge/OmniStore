## 2026-07-27 - Keyboard Navigation, Focus Automation, and Search Dismissal in MD3

**Learning:** To build a highly accessible, pro-grade desktop experience in Flutter matching Material Design 3 guidelines, keyboard accessibility and smart focus transitions must be first-class citizens. Implementing global hotkeys (like `/` or `Ctrl+F` / `Cmd+F` using `CallbackShortcuts`) to seamlessly switch to the Search page must check if any input field (via `EditableText` detection inside `FocusManager.instance.primaryFocus`) is already active, preventing typed character collisions. Furthermore, when entering search screens, auto-focusing the input field (listening to `NavigationController` changes) provides immediate typing utility. Providing consistent dismissal/clearing behaviors (the `Escape` key) across search inputs and detailed screens (using `Navigator.maybePop`) drastically reduces interaction resistance and makes deep navigation pathways effortless to exit.

**Action:** Wrap shell layouts in `CallbackShortcuts` for global search navigation with robust focused-text checking. Auto-focus search fields on screen entry, and always map the `Escape` key to clear active inputs or pop non-embedded pages on desktop.
