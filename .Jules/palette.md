...
## 2026-08-21 - Localized Password Visibility Tooltips

**Learning:** Suffix action buttons on authentication text fields (like password obscure toggles) must provide screen reader tooltips that update dynamically according to state changes (e.g. 'Show password' vs 'Hide password') using centralized `AppLocalizations` strings rather than hardcoded text. This ensures consistent multi-language screen reader accessibility across desktop and mobile platforms.

**Action:** Localized password visibility toggle tooltips on `AccountPage` using newly added `showPassword` and `hidePassword` ARB keys across all supported locales.
