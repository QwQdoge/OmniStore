...
## 2026-07-07 - MD3 Layout Transitions\n\n**Learning:** When using conditionally rendered blocks (like an `AnimatedSwitcher` that handles swapping a loading skeleton for content) inside a layout that can change size drastically, wrapping the `AnimatedSwitcher` in an `AnimatedSize` widget prevents abrupt, jarring jumps in the layout height. This is crucial for maintaining Material 3 motion clarity and preventing render overflow artifacts during state transitions.\n\n**Action:** Wrapped root `AnimatedSwitcher` instances in `AnimatedSize` within `AppDetailsActions` and `AppMainContent` conditionally rendered blocks to ensure smooth, natural layout expansion and contraction.
