...
## 2026-06-18 - AI Error Terminology Consistency
- Extracted AI error localization logic into `_buildAIMarkdown` in `FlutterUI/lib/features/task_manager/presentation/widgets/ai_update_summary_dialog.dart`.
- Replaced hardcoded string \AI failed to summarize.\ with consistent localized error messages (e.g., `aiTimeout`, `aiNoResponse`) to match the behavior of `AIMarkdownDialog` in `ai_dialogs.dart`.
