...
- Identified inconsistency in the display duration of `SnackBar` widgets used for clipboard copy feedback. `FlutterUI/lib/features/explore/presentation/widgets/app_details_shared.dart`, `FlutterUI/lib/features/explore/presentation/widgets/ai_dialogs.dart` and `FlutterUI/lib/features/explore/presentation/widgets/app_details_header.dart` all used an explicit `duration: const Duration(seconds: 4)`.
- Standardized the duration across the application for clipboard feedback by updating `duration: const Duration(seconds: 2)` to the `SnackBar` instantiations in `app_details_shared.dart`, `ai_dialogs.dart` and `app_details_header.dart`. This ensures a unified interaction flow and prevents overlapping or lingering feedback messages.
...
## 2026-08-06 - Localization Terminology Standards Alignment
- Refined and standardized localized terminology across all ARB files (`app_zh.arb`, `app_zh_Hant.arb`, `app_es.arb`).
...
## 2026-08-11 - Dialog Button Consistency: Secondary Actions
- Standardized the secondary/dismissal actions in confirmation and configuration dialogs to use `FilledButton.tonal` instead of `TextButton`, ensuring visual consistency with the app's primary action button pattern (`FilledButton`).
- Updated `AddSourceDialog` (in `add_source_dialog.dart`), `ImportPackagesDialog` (in `import_packages_dialog.dart`), `ActionConfirmDialog`, `AurSecurityDialog`, and `InstallationDecisionDialog` (in `action_dialogs.dart`) to use `FilledButton.tonal`.
