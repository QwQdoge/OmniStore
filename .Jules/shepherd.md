...
## 2026-06-29 - Global SnackBar Consistency: Clipboard Feedback Duration Standardization
- Standardized the duration of `SnackBar` widgets used for clipboard copy feedback across the application to `const Duration(seconds: 2)` instead of 4 seconds.
- Applied this consistency fix to `ai_dialogs.dart`, `app_details_header.dart`, and `app_details_shared.dart` to ensure a unified interaction flow and prevent overlapping feedback messages.
