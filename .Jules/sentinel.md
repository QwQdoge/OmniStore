...
## 2026-07-21 - [Async Cancellation During Multi-Stage Cleanup]

Learning:
During a multi-stage `asyncio` resource cleanup block (like `ResourceCoordinator.cleanup`), if a task cancellation (`asyncio.CancelledError`, which inherits from `BaseException` in Python 3.8+) occurs during one of the inner stages (e.g., closing a network handle), catching only `Exception` allows the cancellation to immediately bubble up, abruptly skipping all subsequent cleanup stages (like deleting temporary files or closing other handles).

Action:
Modified `ResourceCoordinator.cleanup` in `python/core/backend.py` to catch `BaseException` at every cleanup stage. If a cancellation is caught, it is saved in a variable (`cancel_exc = e`), allowing the loop to continue and execute all remaining cleanup logic. The saved cancellation is then re-raised at the very end of the function, ensuring absolute resource reaping without swallowing the termination signal.
