...
1. What did you learn? The task was to replace print statements with logging statements in python/core/search/pacman.py. 2. Were there any patterns or conventions you noticed? The project uses logging.getLogger(__name__) for logging. 3. How could you improve your approach next time? Instead of replacing string literally, I could perhaps use regex or parsing, but simple string replacement works safely for this file size. 4. Is there anything that should be added to the memory? Replacing raw prints with logger warnings/errors is preferred for python backends.
