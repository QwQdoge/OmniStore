...
Python Code Health - Exception Handling: Avoid empty `except Exception:` blocks that silently swallow errors. Replace them with specific exception captures (e.g., `except Exception as e:`) and use standard `logging` (e.g., `logging.error`) to track issues without altering the fallback behavior.
