...
## 2024-11-22 - Smooth Animations for Conditional Elements

**Learning:** Conditionally wrapping an `AnimatedSize` inside an `if` block (e.g., `if (condition) AnimatedSize(...)`) breaks the exit transition because the entire widget is abruptly removed from the tree when the condition evaluates to false, leaving no time to animate the size to zero. To ensure smooth exit transitions, keep the animation wrapper (`SmoothSizeSwitcher` or `AnimatedSize`) permanently in the tree and use a ternary operator to swap its inner child to `SizedBox.shrink()` when the element should be hidden.

**Action:** Replaced conditional `AnimatedSize` in `smooth_progress_bar.dart` with a persistent `SmoothSizeSwitcher` and a ternary operator swapping between a `Container` (`ValueKey('stage')`) and `SizedBox.shrink()` (`ValueKey('empty')`), enabling smooth size transitions. Also upgraded `AnimatedSwitcher` to `SmoothSizeSwitcher` in `account_page.dart` to prevent layout jumping.
