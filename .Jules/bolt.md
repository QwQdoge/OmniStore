...
## 2026-07-29 - PrototypeItem constraints

**Learning:** The `prototypeItem` property in `ListView.builder` forces all children to have the exact same extent in the scroll direction. Using it for variable-width items like `ActionChip`s (e.g., categories or labels) breaks the layout by forcing them into a fixed width, causing either excessive whitespace or truncation.

**Action:** Skipped adding `prototypeItem` in `category_quick_access.dart` and `ai_app_resolver.dart` to preserve variable-width chip layouts, and learned to only apply it to fixed-size lists (like cards or fixed-height rows).
