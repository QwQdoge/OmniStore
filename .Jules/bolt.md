...
## 2026-08-03 - Package Details Caching, Request Coalescing, and Unified Repository Singleton

**Learning:** In Flutter applications, relying solely on MultiProvider dependency injection can be bypassed if other services/routers directly instantiate a repository via its default constructor. Under such circumstances, any in-memory caches (like search result caches or details caches) are created on distinct repository instances, causing silent cache misses and redundant calls. Ensuring the repository class implements a unified Singleton pattern guarantees that all injection and constructor-based accesses leverage the exact same cache instances. Furthermore, implementing request coalescing (storing in-flight futures in a map) alongside standard in-memory caching prevents concurrent duplicate queries from spawning multiple redundant IPC/HTTP/network calls for the same target package details.

**Action:** Refactored `PackageRepository` into a strict singleton using a factory and private internal constructor. Added a Map-based `_detailsCache` with active deduplication coalescing map `_activeDetailsRequests` inside `PackageRepository.getAppDetails`, and registered cache invalidation callbacks (`clearDetailsCacheFor`) inside `details_page.dart` on successful package operations (install/uninstall/update).
