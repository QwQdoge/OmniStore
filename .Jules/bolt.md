...
## 2026-07-31 - SearchPage MediaQuery Optimization

**Learning:** Invoking `MediaQuery.sizeOf(context)` directly inside a `Selector` in `search_page.dart` forces the selector to rebuild the entire search results view on every pixel of window resizing. By extracting `ResponsiveLayoutBuilder` into a shared widget and wrapping the `Selector` with it, we isolate the resize recalculation, skipping the subtree rebuild unless the desktop/mobile threshold (900px) is crossed.

**Action:** Extracted `ResponsiveLayoutBuilder` from `flatpak_store_page.dart` to `responsive_layout_builder.dart`. Wrapped the search results `Selector` in `SearchPage` with `ResponsiveLayoutBuilder` to prevent redundant rebuilds during resizing.
