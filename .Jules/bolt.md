...
## 2026-08-03 - AI Related App Parallelization & Prototype Item Alignment

**Learning:** Parallelizing multiple sequential package queries (e.g., inside loops resolving related apps from AI markdown text in `AIAppResolver`) with `Future.wait` and passing `cancelOngoing: false` to `PackageRepository.searchPackages` significantly speeds up resolution time and prevents concurrent queries from prematurely cancelling each other. Furthermore, when using `prototypeItem` inside `ListView.builder` for virtualized scroll list optimizations, ensure the layout properties of the prototype widget (such as presence/absence of `trailing` elements and vertical height placeholders of `subtitle` structures) perfectly align with the actual `itemBuilder` results. Any structural or dimension mismatch causes layout recalculation, scroll jitter, or inaccurate scrollbar sizing during list virtualization.

**Action:** Optimized `AIAppResolver` by parallelizing search queries with `Future.wait` and passing `cancelOngoing: false`. Standardized prototypeItem layouts in both `InstalledAppList` (added the missing `AppSourceTag` trailing widget) and `InstalledTab` (removed the unused trailing widget placeholder) to perfectly match their actual item builders.
