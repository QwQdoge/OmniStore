...
## $(date +%Y-%m-%d) - AppPackage String Conversions, Learning: Lazy evaluation of `toLowerCase()` properties on models like `AppPackage` prevents expensive O(N) string conversions inside hot-path filtering/search loops. Action: Introduce `late final String nameLower = name.toLowerCase();` on data models to cache conversions.
