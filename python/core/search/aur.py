...
class AurSearch(SearchSource):
    ...
    async def search(self, query: str) -> list:
        ...
        try:
            ...
            tasks = [
                self.session.get(f"{self.api}{query}", timeout=ClientTimeout(total=8)),
            ...
            if not isinstance(resp, aiohttp.ClientResponse):
                print(f"AUR Search: Invalid response type")
                return []
            ...
            for pkg in aur_pkgs:
                ...
                header = f"aur/{name} {pkg['Version']} {status}".strip()
                desc = pkg.get("Description", "") or ""
                
                # 存储为字典，方便后续调用
                final_results.append({
                    "name": name,
                    "last_version": pkg.get("Version", ""),       # 对于搜索，这两个通常一致
                    "source": self.name,
