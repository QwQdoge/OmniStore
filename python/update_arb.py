...
NEW_KEYS_ZH = {
    ...
    "aiEndpointHelper": "Ollama：http://localhost:11434",
    "aiApiKeyHelper": "Ollama：无需密钥；OpenAI：sk-xxx",
    "howToGetApiKey": "如何获取 API 密钥？",
    "howToGetApiKeyDesc": "1. Ollama (本地)：运行 Ollama，无需密钥。2. 云端 (OpenAI)：前往官网创建并填入密钥。",
    "gotIt": "知道了",
    "aiOllamaNote": "使用 Ollama 时，确保其已在后台运行并开启 OLLAMA_ORIGINS=\"*\" 环境变量。",
    "enterStore": "进入商店",
    "nextStep": "下一步",
...
NEW_KEYS_ZH_HANT = {
    ...
    "aiEndpointHelper": "Ollama：http://localhost:11434",
    "aiApiKeyHelper": "Ollama：無需金鑰；OpenAI：sk-xxx",
    "howToGetApiKey": "如何獲取 API 金鑰？",
    "howToGetApiKeyDesc": "1. Ollama (本地)：執行 Ollama，無需金鑰。2. 雲端 (OpenAI)：前往官網建立並填入金鑰。",
    "gotIt": "知道了",
    "aiOllamaNote": "使用 Ollama 時，確保其已在背景執行並開啟了 OLLAMA_ORIGINS=\"*\" 環境變數。",
    "enterStore": "進入商店",
    "nextStep": "下一步",
