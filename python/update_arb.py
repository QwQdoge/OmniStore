...
NEW_KEYS_ZH = {
    ...
    "skip": "跳过",
    "envCheckTitle": "环境检查",
    "envCheckSubtitle": "确保系统准备就绪",
    "envFatalDesc": "当前系统不是 Arch Linux，核心功能受限。",
    "envWarningDesc": "部分必要组件缺失，可自动进行配置。",
    ...
    "aurWarning": "安全警告：AUR 软件包由用户贡献，请确保来源可信。",
    "bootstrapNote": "注意：配置过程可能需要多次输入管理员密码。",
    "feedbackDesc": "如遇问题，请通过 GitHub 提交反馈。",
    "aiAssistant": "AI 助手",
    "aiAssistantDesc": "启用 AI 驱动的搜索、应用解析及错误诊断",
    "aiProviderDesc": "选择您的 AI 模型来源 (本地或云端)",
    "aiEndpointHelper": "Ollama 默认为 http://localhost:11434",
    "aiApiKeyHelper": "Ollama 留空，OpenAI 填入 sk-xxx",
    "howToGetApiKey": "如何获取 API 密钥？",
    "howToGetApiKeyDesc": "1. Ollama (本地)：直接运行，无需密钥。2. 云端 (OpenAI)：前往官网创建并填入 API 密钥。",
    "gotIt": "知道了",
    "aiOllamaNote": "使用 Ollama 时须确保其在后台运行，并启用 OLLAMA_ORIGINS=\"*\" 环境变量。",
    "enterStore": "进入商店",
    "nextStep": "下一步",
...
NEW_KEYS_ZH_HANT = {
    ...
    "skip": "跳過",
    "envCheckTitle": "環境檢查",
    "envCheckSubtitle": "確保系統準備就緒",
    "envFatalDesc": "系統不是 Arch Linux，核心功能受限。",
    "envWarningDesc": "部分必要組件缺失，可自動進行配置。",
    ...
    "aurWarning": "安全警告：AUR 套件由使用者貢獻，請確保來源可信。",
    "bootstrapNote": "注意：配置過程可能需要多次輸入管理員密碼。",
    "feedbackDesc": "如遇問題，請透過 GitHub 提交反饋。",
    "aiAssistant": "AI 助手",
    ...
    "howToGetApiKeyDesc": "1. Ollama (本地)：直接執行，無需金鑰。2. 雲端 (OpenAI)：前往官網建立並填入 API 金鑰。",
    "gotIt": "知道了",
    "aiOllamaNote": "使用 Ollama 時須確保其在背景執行，並啟用 OLLAMA_ORIGINS=\"*\" 環境變數。",
    "enterStore": "進入商店",
    "nextStep": "下一步",
