...
class PackageRepository {
  ...
  Future<Map<String, List<AppPackage>>> getRecommendations({bool forceRefresh = false}) async {
    if (_cachedRecs != null && !forceRefresh) {
      final now = DateTime.now();
      if (_activeFetchFuture == null && (_lastFetchTime == null || now.difference(_lastFetchTime!).inMinutes >= 5)) {
  ...
  Future<Map<String, List<AppPackage>>> _fetchAndCacheRecommendationsImpl() async {
    try {
      if (kIsWeb) {
        try {
